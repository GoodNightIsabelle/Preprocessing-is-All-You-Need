...
class LogParser():
    ...
    def parse(self, filename):
        ...
        use_sequence, matched_types = [], []
        with open('logcluster_input.log', 'w') as fw:
            for idx, line in enumerate(self.df_log['Content']):
                '''if self.rex:
                    for currentRex in self.rex:
                        line = re.sub(currentRex, '', line)'''
                # Add preprocess
                if idx < 2000:
                    line, sequence, matched = preprocess(line, estimation_stage=True)
                    ...
                    line = preprocess(line, estimation_stage=False, use_sequence=use_sequence)
                fw.write(line + '\n')
        try:
